...
def index(request):
    if request.method == 'POST':
        if len(request.POST.getlist('id')) == 10:
            ...
            cases = list(itertools.combinations(entry_list, int(len(entry_list)//2)))
        ...
        if blue > 0 or red > 0 :
            for i in range(blue):
                ...
                for id in request.POST.getlist('blueTeam'):
                    ...
                    league.entry_set.create(member=member, name=member.name, result="승", camp="블루", create_date=timezone.now())
                for id in request.POST.getlist('redTeam'):
                    member = get_object_or_404(Member, pk=id)
                    league.entry_set.create(member=member, name=member.name, result="패", camp="레드", create_date=timezone.now())
            for i in range(red):
                league = League(blue="패", red="승", create_date=timezone.now())
                league.save()
                for id in request.POST.getlist('blueTeam'):
                    member = get_object_or_404(Member, pk=id)
                    league.entry_set.create(member=member, name=member.name, result="패", camp="블루", create_date=timezone.now())
                for id in request.POST.getlist('redTeam'):
                    member = get_object_or_404(Member, pk=id)
                    league.entry_set.create(member=member, name=member.name, result="승", camp="레드", create_date=timezone.now())
            for i in request.POST.getlist('blueTeam'):
                win = Entry.objects.filter(member=i, result="승")
                lose = Entry.objects.filter(member=i, result="패")
                member = get_object_or_404(Member, pk=i)
                member.win = len(win)
                member.lose = len(lose)
                member.total = len(win) + len(lose)
            ...
            context = {'blueTeams' : blueTeams, 'redTeams' : redTeams, 'error' : error, 'blue_win_rate' : blue_win_rate, 'red_win_rate' : red_win_rate}
            return render(request, 'lolteams/league.html', context)
